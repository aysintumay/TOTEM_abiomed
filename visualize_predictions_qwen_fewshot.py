...
for idx in SAMPLE_IDXS:
    ...
    # pl is the true per-step P-level trajectory over the forecast horizon (the action),
    # not a single value -- pass it through as-is rather than collapsing via .mean().
    pl_seq = model.unnorm_pl(torch.as_tensor(np.asarray(pl)).float()).round().long().numpy()

    mean_norm, std_norm = model.predict(context, p_level=pl_seq)
